...
export async function getProduct(slug: string, request: Request) {
  noStore();
...
export async function getWishlistItems(request: Request) {
  const user = await getUser(request);
  if (!user) return [];
...
export async function getOrCreateCart(userId: string, request: Request) {
...
export async function getShoppingBagItems(request: Request) {
  const cart = await getCart(request);
  if (!cart) return [];
...
export async function getShippingAddresses(request: Request) {
  const user = await getUser(request);
  if (!user) return [];
...
export async function createOrderWithItems({
  ...
  const shoppingBagItems = await getShoppingBagItems(request);
...
export async function deleteCart(userId: string, request: Request) {
  const cart = await getCart(request);
...
export async function updateOrderByIdAndUserId({
  ...
  await deleteCart(userId, request);
